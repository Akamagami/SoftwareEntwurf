...
/*--------------------------------------------------------------------------------------------------------------*/
	...
	public void load() {
		for(ObjectData o:csv.initRead()) {
			...
			switch(o.getType()) {
			case BENUTZER: createFromDataBenutzer(data); 
				break;
			case BGRUPPE: createFromDataBGruppe(data);
				break;
			case ELEMENTCATERING: createFromDataCatering(data);
				break;
			case ELEMENTLOCATION: createFromDataLocation(data);
				break;
			case ELEMENTMUSIK: createFromDataMusik(data); 
				break;
			case ELEMENTSONTIGES: createFromDataSonstiges(data);
				break;
			case EVENT:
				break;
			case GRUPPE: createFromDataGruppe(data);
				break;
			case HILFSMITTEL: createFromDataHilfsmittel(data);
				break;
			case KONTAKTINFORMATION: createFromDataKontaktinformation(data);
				break;
...
/*--------------------------------------------------------------------------------------------------------------*/
	...
	private void createFromDataKontaktinformation(String data[]) {
		...
		Kontaktinformation tmp = (Kontaktinformation) this.createObject(ClassType.KONTAKTINFORMATION,params,mainData[1]);
	}
	...
	private void createFromDataCatering(String data[]) {
		...
		for(String s:data) {
			String[] confData = s.split(",");
			if(confData[0].equals(ClassType.KONTAKTINFORMATION.getDisplayName())) {
				tmp.getKontaktInfoList().addKontaktInfo((Kontaktinformation) this.getObject(ClassType.KONTAKTINFORMATION, confData[1]));
			}  else if (confData[0].equals("Picture")){
				Picture p = new Picture(confData[1],confData[2]);
	...
	private void createFromDataZuweisung(String data[]) {
		//create
		String[] mainData = data[0].split(",");
		Object[] params = {this.getObject(ClassType.TEILEVENT, mainData [2]),this.getObject(ClassType.HILFSMITTEL, mainData [3]),mainData[4]};
